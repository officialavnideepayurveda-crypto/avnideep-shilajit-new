...
function initCheckoutUI() {
  ...
  form.addEventListener('submit', async function(e) {
    return; // Order processing handled by main handler (initForm in Script #9)
    // NOTE: Dead code below this return is intentionally removed.
    // The previous implementation here contained the old dedup flag logic
    // (avn_purchase_fired_*) that would block the browser Pixel Purchase
    // event from firing on thank-you.html if re-enabled.
    // All order processing now lives in initForm() in Script #9.
  });
...
function initVideo(){
  ...
  $$('[data-video]').forEach(function(b){
    ...
    function load(){
      ...
      trackFbEvent('ViewContent', {content_name:'AVN-6PRO-001', content_type:'product'});
    ...
    if(m === 'prepaid'){
      ...
      try {
        ...
        if (rzpResult.success) {
          window.location.href = '/thank-you.html?order_id=' + encodeURIComponent(rzpResult.orderId) + '&razorpay_payment_id=' + encodeURIComponent(rzpResult.razorpay_payment_id);
        } else {
...
function initTruecaller() {
  ...
  var tcPhoneInput = document.getElementById('tcPhoneInput');  var tcError = document.getElementById('tcError');
  ...
  tcPhoneInput.addEventListener('keydown', function(e) {
    ...
    if (e.key === 'Enter' && tcBtn && !tcBtn.disabled) {
